...
def get_credentials():
	...
	s.options('https://api.twitter.com/1.1/guest/activate.json', headers={
	...
	s.get("https://twitter.com/sw.js", headers={
		'Accept': '*/*',
		'DNT': '1',
		'Service-Worker': 'script',
	})
	resp= s.post('https://api.twitter.com/1.1/guest/activate.json', headers={
		'authority': 'api.twitter.com',
		'accept': '*/*',
		'accept-language': 'en-US,en;q=0.9',
	...
	if resp.json()["guest_token"]:
		cookies = {**cookies, "gt": resp.json()["guest_token"]}
	return cookies
...
def get_headers():
	...
	return {
		...
		"cookie": "; ".join(f"{k}={v}" for k, v in credentials.items()),
		"dnt": "1",
...
def user_info(username):
	...
	user_info = requests.get(base_url + urllib.parse.urlencode(query_params), headers=get_headers()).json()
